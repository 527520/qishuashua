...
import com.wqa.qishuashua.model.enums.UserRoleEnum;
...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements UserService {
    ...
    @Override
    public long userRegister(String userAccount, String userPassword, String checkPassword, String phoneNumber, String userName) {
        // 1. 校验
    ...
    @Override
    public boolean isAdminOrVip(User loginUser) {
        return isAdmin(loginUser) || UserConstant.VIP_ROLE.equals(loginUser.getUserRole());
    }
    ...
    public boolean addUserSignIn(long userId) {
        ...
        String redisKey = RedisConstant.getUserSignInRedisKey(nowData.getYear(), userId);
        // 获取到Redis的BitMap
        RBitSet signInBitSet = redissonClient.getBitSet(redisKey);
        // 获取当前日期是当年的第几天，作为偏移量
        int offset = nowData.getDayOfYear();
        // 查询当前是否签到
        boolean isSignIn = signInBitSet.get(offset);
        if (!isSignIn) {
            // 设置签到
